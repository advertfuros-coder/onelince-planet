                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div>
                        <div className="flex items-center gap-2 mb-2">
                            ...
                            <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest bg-blue-50 px-3 py-1 rounded-full">Order Hub</span>
                        </div>
                        <h1 className="text-4xl font-black text-gray-900 tracking-tighter">Fulfillment Pipeline</h1>
                    ...
                    <div className="flex items-center gap-2 overflow-x-auto pb-2 lg:pb-0 w-full lg:w-auto invisible-scrollbar">
                        {['all', 'pending', 'processing', 'packed', 'shipped', 'delivered', 'cancelled'].map(status => (
                            <button
                                ...
                                className={`px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-tight transition-all shrink-0 ${filter === status
                    ...
                    {selectedOrderIds.length > 0 && (
                        ...
                        >
                            <div className="flex items-center gap-4 pr-8 border-r border-white/10">
                                ...
                                <div>
                                    <p className="text-[10px] font-black uppercase tracking-widest text-blue-400">Units Selected</p>
                                    <p className="text-xs font-bold text-white/60">Ready for batch processing</p>
                                </div>
                            </div>

                            <div className="flex items-center gap-3">
                                <span className="text-[10px] font-black uppercase tracking-widest text-white/40 mr-2">Transition To:</span>
                    ...
                    {!loading && filteredOrders.length > 0 && (
                        <div className="flex items-center justify-between px-6 py-2">
                            <label className="flex items-center gap-3 cursor-pointer group">
                                ...
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Select All {filteredOrders.length} Units</span>
                            </label>
                        </div>
                    )}
                    {loading ? (
                        <div className="flex flex-col items-center justify-center py-32 space-y-4">
                            ...
                            <p className="text-gray-400 font-black uppercase tracking-widest text-[10px]">Syncing Logistics Console...</p>
                        </div>
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-black text-gray-900 tracking-tighter">No Units in Pipeline</h3>
            ...
            >
                ...
                <div className="flex justify-between items-start mb-6 pl-8">
                    <div className="space-y-1">
                        <div className="flex items-center gap-2">
                            <h4 className="text-lg font-black text-gray-900 tracking-tight" onClick={() => onSelect(order)}>{order.orderNumber}</h4>
                            <ArrowUpRight size={14} className="text-gray-300 group-hover:text-blue-600 transition-colors" />
                        </div>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{new Date(order.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</p>
                    </div>
                ...
                <div className="space-y-4 mb-8 pl-8">
                    ...
                    <div className="flex items-center gap-3 text-xs font-bold text-gray-600">
                        ...
                        <span>₹{(order.pricing?.total || 0).toLocaleString()} <span className="text-[10px] text-gray-400 font-bold uppercase ml-1">Paid • {order.paymentMethod || 'Razorpay'}</span></span>
                    </div>
                    <div className="flex items-center gap-3 text-xs font-bold text-gray-600">
                ...
                <div className="grid grid-cols-2 gap-3 pl-8">
                    <button
                        ...
                        className="py-3 bg-gray-50 text-gray-900 rounded-xl text-[10px] font-black uppercase hover:bg-gray-100 transition-all"
                    >
                        Detailed Log
                    </button>
                    {order.status === 'pending' && (
                        <button
                            ...
                            disabled={updating}
                            className="py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/10 active:scale-95 disabled:opacity-50"
                        >
                            Confirm Unit
                        </button>
                    )}
                    {order.status === 'confirmed' && (
                        <button
                            ...
                            disabled={updating}
                            className="py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-indigo-700 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                        >
                            Start Prep
                        </button>
                    )}
                    {order.status === 'processing' && (
                        <button
                            ...
                            disabled={updating}
                            className="py-3 bg-violet-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-violet-700 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                        >
                            Seal Pack
                        </button>
                    )}
    ...
    return (
        ...
        >
            <div className="flex items-center gap-6">
                ...
                <div className="space-y-1">
                    <div className="flex items-center gap-3">
                        <h4 className="text-base font-black text-gray-900 tracking-tight" onClick={() => onSelect(order)}>{order.orderNumber}</h4>
                        <div className={`px-2 py-0.5 rounded-lg border text-[9px] font-black uppercase tracking-tighter shrink-0 ${order.status === 'delivered' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                            order.status === 'pending' ? 'bg-amber-50 text-amber-600 border-amber-100' :
                                'bg-blue-50 text-blue-600 border-blue-100'
                            }`}>
                            {config.label}
            ...
            <div className="flex items-center justify-between lg:justify-end gap-8 border-t lg:border-none pt-4 lg:pt-0">
                <div className="text-right">
                    <p className="text-sm font-black text-gray-900 tracking-tight">₹{(order.pricing?.total || 0).toLocaleString()}</p>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{totalItems} Units</p>
                </div>
                ...
                <div className="flex items-center gap-2">
                    ...
                    {order.status === 'pending' && (
                        <button
                            onClick={() => quickStatusUpdate('confirmed')}
                            disabled={updating}
                            className="px-6 py-3 bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-blue-700 transition-all shadow-lg shadow-blue-500/10 active:scale-95 disabled:opacity-50"
                        >
                            Confirm Dispatch
                        </button>
                    )}
                    {order.status === 'confirmed' && (
                        <button
                            onClick={() => quickStatusUpdate('processing')}
                            disabled={updating}
                            className="px-6 py-3 bg-indigo-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-indigo-700 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                        >
                            Move to Prep
                        </button>
                    )}
                    {order.status === 'processing' && (
                        <button
                            onClick={() => quickStatusUpdate('packed')}
                            disabled={updating}
                            className="px-6 py-3 bg-violet-600 text-white rounded-xl text-[10px] font-black uppercase hover:bg-violet-700 transition-all shadow-lg active:scale-95 disabled:opacity-50"
                        >
                            Seal Manifest
                        </button>
                    )}
        ...
        >
            ...
            >
                <div className="p-8 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-2 py-0.5 rounded tracking-widest uppercase">Unit Dossier</span>
                            <h2 className="text-2xl font-black text-gray-900 tracking-tighter">{order.orderNumber}</h2>
                        </div>
                        <p className="text-gray-400 font-bold text-xs uppercase tracking-tight">Initialized on {new Date(order.createdAt).toLocaleString()}</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="flex-1 overflow-y-auto p-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div className="space-y-8">
                            <section>
                                <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Consignee Intelligence</h5>
                                <div className="bg-gray-50 rounded-3xl p-6 border border-gray-100">
                                    <div className="flex items-start gap-4 mb-6 pb-6 border-b border-gray-200/50">
                                        ...
                                        <div>
                                            <p className="text-sm font-black text-gray-900 mb-0.5">{order.shippingAddress?.name || order.customer?.name}</p>
                                            <p className="text-xs font-bold text-gray-400 tracking-tight">{order.customer?.email || 'Authenticated Portal User'}</p>
                                        </div>
                                    </div>
                            ...
                            <section>
                                <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Unit Manifest ({order.items?.length || 0})</h5>
                                    ...
                                    {order.items?.map((item, idx) => (
                                        <div key={idx} className="flex items-center gap-4 bg-white border border-gray-100 p-3 rounded-2xl">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[11px] font-black text-gray-900 truncate tracking-tight">{item.name || item.productId?.name}</p>
                                                <p className="text-[10px] font-bold text-gray-400 uppercase">QTY: {item.quantity} • ₹{item.price?.toLocaleString()}</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-[11px] font-black text-blue-600">₹{(item.price * item.quantity).toLocaleString()}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="space-y-8">
                            <section>
                                <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Financial Breakdown</h5>
                                <div className="bg-[#0A1128] rounded-3xl p-8 text-white relative overflow-hidden">
                                    ...
                                    <div className="relative z-10 space-y-4">
                                        <div className="flex justify-between text-white/50 text-[11px] font-black uppercase tracking-widest">
                                            <span>Subtotal</span>
                                            <span>₹{(order.pricing?.subtotal || 0).toLocaleString()}</span>
                                        </div>
                                        <div className="flex justify-between text-white/50 text-[11px] font-black uppercase tracking-widest pb-6 border-b border-white/10">
                                            <span>Logistics</span>
                                            <span>{order.pricing?.shippingFee > 0 ? `₹${order.pricing.shippingFee}` : 'FREE'}</span>
                                        </div>
                                        <div className="flex justify-between items-center bg-white/5 p-4 rounded-2xl">
                                            <span className="text-xs font-black uppercase tracking-widest">Net Revenue</span>
                                            <span className="text-xl font-black text-blue-400">₹{(order.pricing?.total || 0).toLocaleString()}</span>
                                        </div>
                                        <div className="flex items-center gap-2 pt-2">
                                            <CreditCard size={14} className="text-blue-400" />
                                            <span className="text-[10px] font-black text-white/40 uppercase tracking-widest">Authenticated via {order.paymentMethod || 'SECURE PORTAL'}</span>
                                        </div>
                                    </div>
                                </div>
                            </section>
                            ...
                            <section>
                                <h5 className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Shipping Control</h5>
                                <div className="bg-white border-2 border-dashed border-gray-100 rounded-3xl p-6">
                                    <p className="text-[10px] font-bold text-gray-400 uppercase mb-4">Carrier Documentation</p>
                                    <button
                                        onClick={handleGenerateManifest}
                                        disabled={!order.shipping?.shipmentId}
                                        className="w-full py-4 bg-gray-50 border border-gray-100 rounded-2xl flex items-center justify-center gap-3 font-bold text-gray-600 hover:bg-gray-100 transition-all disabled:opacity-50"
                                    >
                                        <Truck size={18} />
                ...
                <div className="p-8 bg-gray-50/50 border-t border-gray-100 flex items-center justify-between">
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Dispatch Status: <span className="text-blue-600">{order.status.replace('_', ' ')}</span></p>
                    <div className="flex gap-3">
                        <button
                            onClick={handleGenerateInvoice}
                            className="px-6 py-3 bg-white border border-gray-200 text-gray-900 rounded-2xl text-[10px] font-black uppercase transition-all hover:bg-white hover:shadow-lg active:scale-95"
                        >
                            Generate Invoice
                        </button>
                        <button
                            onClick={onClose}
                            className="px-8 py-3 bg-gray-900 text-white rounded-2xl text-[10px] font-black uppercase transition-all hover:bg-black shadow-xl active:scale-95"
                        >
                            ACKNOWLEDGE
                        </button>
                    </div>
