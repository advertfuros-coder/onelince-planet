...
export async function POST(request) {
  try {
    ...
    if (!paymentMethod) {
      return NextResponse.json({ success: false, message: 'Payment method required' }, { status: 400 })
    }
    ...
    for (const item of items) {
      ...
      if (availableStock < item.quantity) {
        return NextResponse.json(
          { success: false, message: `Only ${availableStock} units available for ${product.name}` },
          { status: 400 }
        )
      }
    ...
    // Apply discount if coupon code provided (you can implement coupon logic here)
    let discount = 0
    // if (couponCode) {
    ...
    const order = await Order.create({
      ...
      payment: {
        ...
        status: paymentMethod === 'cod' ? 'pending' : 'pending', // Will be updated after payment gateway response
      },
      timeline: [{
        status: 'pending',
    ...
    const populatedOrder = await Order.findById(order._id)
      ...
      .populate('items.seller', 'businessName name email')
