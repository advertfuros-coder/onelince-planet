...
export async function GET(request) {
  try {
    ...
    if (productId) {
      ...
      const product = await Product.findOne({ _id: productId, sellerId: decoded.userId })
      ...
      return NextResponse.json({
        ...
        performance: {
          ...
          summary: {
            ...
            totalOrders: performanceData.reduce((sum, p) => sum + p.totalOrders, 0),
            totalUnits: performanceData.reduce((sum, p) => sum + p.totalUnits, 0),
            averageRating: performanceData.reduce((sum, p) => sum + p.rating, 0) / performanceData.length || 0
          }
        }
      })
    }
