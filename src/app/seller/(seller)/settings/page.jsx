                  ...
                  {tabs.map((tab) => (
                     ...
                     >
                        <div className="flex items-center gap-4 relative z-10">
                           ...
                           <div>
                              <p className="text-[11px] font-semibold uppercase tracking-widest leading-none mb-1">{tab.name}</p>
                              <p className={`text-[9px] font-semibold ${activeTab === tab.id ? 'text-white/40' : 'text-gray-300'}`}>{tab.description}</p>
                           </div>
                        </div>
                              ...
                              <div className="space-y-10">
                                 <div className="flex items-center gap-8 p-8 bg-gray-50/50 rounded-[2.5rem] border border-gray-100">
                                    <div className="relative group">
                                       <div className="w-24 h-24 rounded-[2rem] bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center text-3xl font-semibold text-white shadow-xl">
...
function SettingInput({ label, value, onChange, placeholder, type = "text" }) {
   return (
      <div className="space-y-2 group">
         <label className="text-[10px] font-semibold text-gray-400 uppercase tracking-widest px-2 group-focus-within:text-blue-600 transition-colors">{label}</label>
         <input
            type={type}
            value={value || ''}
            onChange={(e) => onChange(e.target.value)}
            placeholder={placeholder}
            className="w-full px-6 py-4 bg-gray-50/50 border border-transparent rounded-[1.5rem] text-[13px] font-semibold placeholder:text-gray-300 focus:bg-white focus:border-blue-100 focus:ring-4 focus:ring-blue-500/5 transition-all outline-none"
         />
      </div>
   )
}
