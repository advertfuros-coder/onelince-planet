        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-8 bg-[#F8FAFC]">
                <div className="flex flex-col items-center gap-4">
                    ...
                    <p className="text-xs font-semibold uppercase tracking-widest text-slate-400">Loading Claims...</p>
                </div>
            </div>
        )
    }
        ...
        <div className="min-h-screen bg-[#F8FAFC] pb-20">
            <div className="max-w-[1400px] mx-auto p-6 lg:p-10 space-y-10">
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div>
                        <h1 className="text-4xl font-semibold text-slate-900 tracking-tight">Claims Manager <span className="text-rose-600">.</span></h1>
                        <p className="text-slate-500 font-semibold text-sm mt-1">Review return requests and manage quality disputes.</p>
                    </div>
                </div>
                        ...
                        <div className="bg-[#0F172A] rounded-[2.5rem] p-8 text-white relative overflow-hidden h-full">
                            <div className="relative z-10">
                                <div className="flex items-center gap-2 text-rose-400 mb-4">
                                    <ShieldAlert size={18} />
                                    <span className="text-[10px] font-semibold uppercase tracking-[0.2em]">Dispute Radar AI</span>
                                </div>
                                <h3 className="text-lg font-semibold mb-4 tracking-tight">Return Fraud Detection</h3>
                                ...
                                <div className="flex items-center justify-between text-[10px] font-semibold uppercase tracking-widest text-slate-500 border-t border-slate-800 pt-6">
                                ...
                                <div className="flex bg-slate-50 p-1 rounded-2xl border border-slate-100">
                                    {['requested', 'processing', 'resolved', 'all'].map((tab) => (
                                        <button
                                            ...
                                            className={`px-6 py-2 rounded-xl text-[10px] font-semibold uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-slate-900 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'
                                                }`}
                                        >
                                            {tab}
                                        </button>
                            ...
                            <div className="divide-y divide-slate-50">
                                {filteredReturns?.length > 0 ? (
                                                ...
                                                <div className="flex items-center gap-6">
                                                    ...
                                                    <div>
                                                        <div className="flex items-center gap-3 mb-1">
                                                            <span className="text-[10px] font-semibold text-rose-600 uppercase tracking-widest bg-rose-50 px-2 py-0.5 rounded">Claim #{item.orderNumber}</span>
                                                            <span className="text-[9px] font-semibold text-slate-400 uppercase tracking-widest">{new Date(item.requestedAt).toLocaleDateString()}</span>
                                                        </div>
                                                        <h4 className="text-sm font-semibold text-slate-900 mb-1">{item.reason}</h4>
                                                        <p className="text-[10px] font-semibold text-slate-400 uppercase tracking-widest">Requested by: <span className="text-slate-700">{item.customer}</span></p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-6 text-right">
                                                    <div>
                                                        <p className="text-xs font-semibold text-slate-900">₹{item.refundAmount.toLocaleString()}</p>
                                                        <p className="text-[9px] font-semibold text-slate-400 uppercase tracking-widest">Est. Refund</p>
                                                    </div>
                                    ...
                                    <div className="py-20 text-center flex flex-col items-center gap-4">
                                        ...
                                        <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">All clear: No claims found</p>
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            {selectedReturn ? (
                                ...
                                >
                                    <div className="flex items-center gap-4 border-b border-slate-50 pb-6">
                                        ...
                                        <div>
                                            <h3 className="text-xs font-semibold text-slate-900 uppercase tracking-widest">Claim Lifecycle</h3>
                                            <p className="text-[10px] font-semibold text-slate-400 uppercase">Manage workflow stages</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-6">
                                            ...
                                            <div className="space-y-6">
                                                <div className="bg-amber-50 p-6 rounded-3xl border border-amber-100 flex gap-4">
                                                    ...
                                                    <div className="space-y-1">
                                                        <p className="text-[10px] font-semibold text-amber-900 uppercase tracking-widest">Customer Note</p>
                                                ...
                                                <div className="grid grid-cols-1 gap-3">
                                                    <button
                                                        ...
                                                        className="w-full py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-semibold uppercase tracking-widest hover:bg-black transition-all shadow-xl shadow-slate-200"
                                                    >
                                                        Approve Return
                                                    </button>
                                                    <button
                                                        ...
                                                        className="w-full py-4 bg-rose-50 text-rose-600 rounded-2xl text-[10px] font-semibold uppercase tracking-widest hover:bg-rose-100 transition-all"
                                                    >
                                                        Reject Claim
                                                    </button>
                                                </div>
                                        ...
                                        {selectedReturn.status === 'approved' && (
                                            <div className="text-center space-y-6">
                                                <div className="p-10 bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 flex flex-col items-center gap-4">
                                                    ...
                                                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest leading-relaxed">Awaiting arrival of the physical asset from the logistics network.</p>
                                                </div>
                                                <button
                                                    onClick={() => handleAction(selectedReturn.id, 'RECEIVED')}
                                                    className="w-full py-4 bg-emerald-600 text-white rounded-2xl text-[10px] font-semibold uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-xl shadow-emerald-200"
                                                >
                                                    Mark as Received
                                                </button>
                                            </div>
                                        ...
                                        {selectedReturn.status === 'received' && (
                                            <div className="space-y-6">
                                                <h4 className="text-[10px] font-semibold text-slate-900 uppercase tracking-widest">Quality Inspector</h4>
                                                <div className="grid grid-cols-1 gap-3">
                                                    <button
                                                    ...
                                                    <button
                                                        ...
                                                        className="w-full py-4 bg-rose-600 text-white rounded-2xl text-[10px] font-semibold uppercase tracking-widest hover:bg-rose-700 transition-all shadow-xl shadow-rose-200"
                                                    >
                                                        Fail Quality Check
                                                    </button>
                                                </div>
                                        ...
                                        {['quality_passed', 'refunded', 'rejected'].includes(selectedReturn.status) && (
                                            <div className="text-center space-y-4 py-8">
                                                ...
                                                <h4 className="text-sm font-semibold text-slate-900 uppercase tracking-widest">Case Resolved</h4>
                                                <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-widest leading-relaxed">The claiming process for this unit is finalized and archived.</p>
                                            </div>
                                        )}
                                    </div>
                                    ...
                                    <div className="pt-8 border-t border-slate-50 space-y-4">
                                        <button className="flex items-center justify-between w-full p-4 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-all">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] font-semibold text-slate-600 uppercase">View Order Detail</span>
                                            </div>
                                        ...
                                        <button className="flex items-center justify-between w-full p-4 bg-slate-50 rounded-2xl hover:bg-slate-100 transition-all">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] font-semibold text-slate-600 uppercase">Contact Support</span>
                                            </div>
                                            <ChevronRight size={14} className="text-slate-300" />
                                        </button>
                                    </div>
                                ...
                                <div className="h-full bg-slate-50/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 flex flex-col items-center justify-center p-12 text-center grayscale opacity-50">
                                    ...
                                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest leading-relaxed">Select a claim from the list to initiate inspection.</p>
                                </div>
                            )}
                        </AnimatePresence>
                    </div>
