...
export default function AdminAnalyticsPage() {
  ...
  useEffect(() => {
    if (!authLoading) {
      if (token) {
        ...
        setLoading(false)
        // Optionally redirect here if needed, or show unauthorized state
      }
    }
  }, [token, period, authLoading])
  ...
  async function fetchAnalytics(predict = false) {
    try {
      ...
      setError(error.message || 'Error occurred while loading analytics')
    } finally {
      setLoading(false)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-red-50">
        <div className="text-center p-8 bg-white rounded-lg shadow-xl border border-red-200">
          ...
          <h2 className="text-2xl font-semibold text-gray-800 mb-2">Error Loading Analytics</h2>
          <p className="text-red-600 mb-6">{error}</p>
          <button
            onClick={() => fetchAnalytics()}
            className="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors flex items-center justify-center mx-auto space-x-2"
          >
            <FiRefreshCw />
            <span>Try Again</span>
          </button>
    ...
    <div className="space-y-6 pb-12">
      ...
      {predictions && (
        <div className="bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 rounded-xl border-2 border-purple-200 p-8 shadow-xl">
          <div className="flex items-center space-x-4 mb-6">
            ...
            <div>
              <h2 className="text-2xl font-semibold text-gray-900">AI-Powered Business Insights</h2>
        ...
        <div className="space-y-6">
          ...
          {regional.ordersByRegion && regional.ordersByRegion.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h3 className="text-xl font-semibold text-gray-900 mb-4">📊 Regional Performance Table</h3>
      ...
      {activeTab === 'pricing' && pricing && (
        <div className="space-y-6">
          ...
          {pricing.productPricingAnalysis && pricing.productPricingAnalysis.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h3 className="text-xl font-semibold text-gray-900 mb-4">💎 Product Pricing Strategy</h3>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                    ...
                    {safeSlice(pricing.productPricingAnalysis, 0, 20).map((product, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm text-right">
                          <span className={`font-semibold ${product.profitMargin > 40 ? 'text-green-600' :
      ...
      {activeTab === 'customers' && customers && (
        <div className="space-y-6">
          ...
          {customers.customerLifetimeValue && customers.customerLifetimeValue.length > 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              <h3 className="text-xl font-semibold text-gray-900 mb-4">🏆 Top Customers by Lifetime Value</h3>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {products.fastMovingProducts && products.fastMovingProducts.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h3 className="text-xl font-semibold text-gray-900 mb-4">🔥 Fast Moving Products</h3>
            ...
            {products.slowMovingProducts && products.slowMovingProducts.length > 0 && (
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                <h3 className="text-xl font-semibold text-gray-900 mb-4">❄️ Slow Moving Products</h3>
                <div className="space-y-3">
                  {safeSlice(products.slowMovingProducts, 0, 10).map((product, idx) => (
                    <div key={idx} className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg">
                      <div className="flex items-center space-x-3">
                        <span className="font-semibold text-yellow-600">⚠️</span>
                        <div>
                          <p className="font-semibold text-gray-900">{product.product?.name || 'Unknown Product'}</p>
